...
- Cadena Groq → Gemini → repertorio local. Los modelos del plan gratuito de Groq viven en constantes (`GROQ_CALIDAD`, `GROQ_RAPIDO`): **no** la familia llama, que pasó a Enterprise en agosto de 2026 y devolvía 404 en cada mensaje.
...
- **Memoria de fallos en tres niveles**: (1) modelo caído por 6 h (404/400 → no vuelve a intentarse), (2) proveedor en pausa según el error (401/403 → 1 h, 429 → 1 min, sin modelos → 30 min) y (3) listado de modelos fallido → no se reintenta por 10 min. Sin esto, un modelo retirado o una clave sin permiso costaban un viaje de red fallido **por mensaje**.
